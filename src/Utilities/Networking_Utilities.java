 ...
 */
public class Networking_Utilities {
    ...
    public static String getMACAddress(String interfaceName) {
        try {
            ...
            for (NetworkInterface intf : interfaces) {
                ...
                if (mac == null) {
                    AppTester.killApplication("Didn't work");
                    return "";
                }
            ...
            AppTester.killApplication("That didn't work");
    ...
    public static InetAddress[] getAllReachableViableNonLoopBackInetAddresses() throws Exception {
        ...
        try {
            ...
            AppTester.printEx("Failed to get network interfaces.", se);
            AppTester.check(ipAddresses.isEmpty());
        ...
        for (final NetworkInterface intf : interfaces) {
            try {
                ...
                AppTester.printEx("Failed to access network interface " + intf.getDisplayName(), se);
                continue; // keep going.
            }
            ...
            for (InetAddress addr : addrs) {
                if (addr == null || addr.isLoopbackAddress()) {
                    ...
                    AppTester.check(addr.getHostAddress().toString().contains(":")
                ...
                if (defaultAddressOrNull != null) {
                    if (defaultAddressOrNull.isSiteLocalAddress()) {
                        ...
                        if (isReachable) {
                            ...
                            InetAddress[] inetAddressArr = new InetAddress[ipAddresses.size()];
                            inetAddressArr = ipAddresses.toArray(inetAddressArr);
                            AppTester.check(inetAddressArr.length == ipAddresses.size(), "Failed to transfer addresses");
                            AppTester.check(inetAddressArr.length == 1, "I only inserted one element.");
                            return inetAddressArr;
                        }
                    }
                }
            } catch (UnknownHostException uhe) {
                AppTester.printEx(uhe, "Failed to get local host.");
            }
        }*/
    ...
    private static void checkIfNumLinkLocalAddressesIsReasonable(final InetAddress[] inetAddresses) {
        ...
        for (InetAddress addr : inetAddresses) {
            AppTester.check(!(addr == null), "No null addresses");
            if (addr.getHostAddress().toString().contains(":")) {
                if (addr.isLinkLocalAddress()) {
                ...
                AppTester.killApplication("All ip addresses must contain either a colon or a dot.");
            }
        }
        AppTester.check(numSiteLocalIpv4Addresses == 0 || numSiteLocalIpv4Addresses == 1, "Too many link local ipv4 addresses");
        AppTester.check(numSiteLocalIpv6Addresses == 0 || numSiteLocalIpv6Addresses == 1, "Too many link local ipv6 addresses");
    }
}
